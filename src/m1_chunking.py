...
def _split_by_size(text: str, size: int) -> list[str]:
    """Split text thành các đoạn ≤ size chars, không cắt giữa từ."""
    words = text.split()
    if not words:
        return []
    chunks = []
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    def _flush():
        ...
        if combined:
            section_name = re.sub(r'^#{1,3}\s*', '', current_header).strip()
            chunks.append(Chunk(text=combined, metadata={**metadata, "section": section_name, "strategy": "structure"}))
